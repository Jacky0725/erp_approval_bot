...
- P4/P5 定向验证：规则与复核 41 项、审计与审批流 85 项、任务管理与 UI 请求 48 项全部通过。
...
4. **已确认策略：** 2026-09-11 用户确认所有“未知类”自动写入 ERP。规则引擎、结构化规则表、迁移器、旧记忆/V2 结果归一化、写入候选和人工队列入口已统一；已有队列数据未迁移或删除。实际写入仍受 dry-run、写入模式、ERP 映射和保存验证门控。
